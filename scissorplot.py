...
class ConventionalTail(GeomBase):
    ...
    #  Sh/S inputs
    x_cg = Input(-4.0)   #  NEED THIIS INPUT FROM CG SCRIPT FOR CURRENT AIRCRAFT!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    C_L_ah = Input(0.9)     #  This is the maximum lift coeficcent of the wing at 1.2*V_s IMPORT FROM WING
    C_mac = Input(-0.17)    #  This is the C_m of the wing from AVL IMPORT FROM WING
    Cla_w = Input(5.1)      #  This is the lift curve slope of the wing from AVL. IMPORT FROM WING
    delta_xcg = Input(0.3)  #  This is the change in the cg location due to dropping a payload. IMPORT FROM CG EST
    Cla_h = Input(4.8)      # This is the lift curve slope of the tail. IMPORT FROM TAIL WING INSTANTIATION
#  TODO In main code, get cla_h from AVL using wing primitive.
   ...
   #     return Cla_h
    ...
    def shs_stability(self):
        ...
        for i in range(0,len(self.xcg_range)):
            if self.configuration is 'Conventional':
                ...
                shs_canard = (self.xcg_range[i] / ((self.Cla_h / self.Cla_w) * (self.lhc_canard) * ((self.VhV_canard) ** 2))) - ((self.x_ac - self.SM) / ((self.Cla_h / self.Cla_w) * (self.lhc_canard) * ((self.VhV_canard) ** 2)))
    ...
    def shs_control(self):
        ...
        for i in range(0, len(self.xcg_range)):
            if self.configuration is 'Conventional':
                shs_conv = (self.xcg_range[i]/((self.Cl_h/self.C_L_ah)*(self.lhc)*((self.VhV_conv)**2))) + (((self.C_mac/self.C_L_ah) - self.x_ac) / ((self.Cl_h/self.C_L_ah) *(self.lhc) * ((self.VhV_conv) ** 2)))
                shs_c.append(shs_conv)
            else:
                shs_canard = (self.xcg_range[i] / ((self.Cl_h / self.C_L_ah) * (self.lhc_canard)) * ((self.VhV_canard) ** 2)) + (((self.C_mac / self.C_L_ah) - self.x_ac) / ((self.Cl_h / self.C_L_ah) * (self.lhc_canard) * ((self.VhV_canard) ** 2)))
    ...
    def shs_req(self):
        ...
        if self.configuration is 'Conventional':
            ...
            shs_req = (self.delta_xcg + self.SM - (self.C_mac / self.C_L_ah)) / ((((self.Cla_h / self.Cla_w) * 1) - (self.Cl_h / self.C_L_ah)) * (self.VhV_canard ** 2) * self.lhc_canard)
    ...
    def scissorplot(self):
        ...
        axes = plt.gca()
        axes.set_ylim([-2.0, max(self.shs_stability)])
